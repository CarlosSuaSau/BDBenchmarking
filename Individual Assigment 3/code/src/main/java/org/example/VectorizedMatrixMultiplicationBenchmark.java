...
public class VectorizedMatrixMultiplicationBenchmark {
    ...
    public double[][] vectorizedMultiplication() {
        return VectorizedMatrixMultiplication.multiply(A, B);
    }

    @TearDown(Level.Trial)
    public void saveResults() {
        try (FileWriter writer = new FileWriter("vectorized_benchmark_results_pre.txt", true)) {
            MemoryMXBean memoryBean = ManagementFactory.getMemoryMXBean();
            MemoryUsage heapUsage = memoryBean.getHeapMemoryUsage();
            long usedMemory = heapUsage.getUsed();

            writer.write("ApacheCommons Matrix Size: " + matrixSize +
